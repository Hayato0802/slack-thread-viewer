...
2. **OAuth & Permissions** > **User Token Scopes** に以下を追加:
   ...
   - `users:read`
3. **Redirect URLs** は拡張インストール後に設定（手順4参照）

### 2. リポジトリをクローン

```bash
git clone https://github.com/Hayato0802/slack-thread-viewer.git
```
...
4. 拡張の設定画面（オプション）を開き、下部に表示される **Redirect URL** をSlack Appの **Redirect URLs** に追加

### 5. ログイン

1. Slackを開いた状態で拡張アイコンをクリック
2. サイドパネルの「Slackでログイン」ボタンをクリック
3. 認証完了後、自動でスレッド一覧が表示されます
...
1. 管理者がSlack Appを作成（上記スコープを設定）
2. `config.js` にClient ID/Secretを記入
3. 拡張をZIPまたはChrome Web Store（限定公開）で配布
4. ユーザーは「Slackでログイン」ボタン1回で利用開始
